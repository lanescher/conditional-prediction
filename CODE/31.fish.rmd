...
# sidenote: For the two species that don't gain improvement from conditioning, are they fit in the model worse than other species?

```{r}
rmspe_is <- out_is$fit$rmspeBySpec
rmspe_oos <- out_oos$fit$rmspeBySpec

rmspe <- data.frame(species = names(rmspe_is),
                    is = rmspe_is,
                    oos = rmspe_oos)
...
b <- ggplot(filter(full, metric == "percDiff")) + 
...
ggpubr::ggarrange(a, b, nrow = 1,
                  labels = "auto",
                  widths = c(0.3, 1),
                  label.x = c(0.13, 0), label.y = .95,
                  font.label = list(size = 10),
                  align = "h")


ggsave(filename = "../OUT/figures/metrics.png",
       height = 110, width = 180, units = "mm", dpi = 600)
...
base1 <- ggplot() + 
  ...
  scale_fill_gradientn(colors = c("white", "yellow", "firebrick2"), 
...
b <- base2 +
  geom_sf(data = filter(strat_sub, 
          ...
          aes(group = as.factor(value), fill = value)) +
  coord_sf(xlim=c(-74, -65), ylim=c(40,46), expand = TRUE) +
  labs(title = "Error from traditional prediction") #+
...
c <- base2 +
  geom_sf(data = filter(strat_sub, 
          ...
          aes(group = as.factor(value), fill = value)) +
  coord_sf(xlim=c(-74, -65), ylim=c(40,46), expand = TRUE) +
  labs(title = "Error from conditional prediction") #+
  #theme(axis.text = element_blank())

# add panel comparing b and c? No.
# tmp <- strat_sub %>%
#   filter(name %in% c("sub_con", "sub_uncon"), 
#                         strat_sub$spp == "ATLANTIC.COD",
#                         strat_sub$year == 2012) %>%
#   select(!c("conditional", "unconditional", "name")) %>%
#   pivot_wider(names_from = "name1", values_from = "value") %>%
#   mutate(diff = sub_uncon - sub_con)
# 
# d <- base2 +
#   geom_sf(data = tmp,
#           aes(fill = diff)) +
#   coord_sf(xlim=c(-74, -65), ylim=c(40,46), expand = TRUE) +
#   labs(title = "Traditional prediction error compared to \nconditional prediction error") #+
#   #theme(axis.text = element_blank())


### I can't get panels lined up if they have separate legends, so I'm
#   arranging this manually :(

# this makes the figure with panels aligned
ggpubr::ggarrange(a, b, c, ncol = 1,
                  labels = "auto", 
                  label.y = 1, label.x = 0.15,
                  legend = "none",
                  font.label = list(size = 10),
                  align = "hv")
...
# this makes the legends
top <- ggpubr::ggarrange(a, labels = "a", legend = "right")
bottom <- ggpubr::ggarrange(b, c, labels = c("b", "c"), 
                            legend = "right", ncol = 1,
                            common.legend = T)

ggpubr::ggarrange(top, bottom, ncol = 1, heights = c(1, 2))

ggsave("../OUT/figures/codmaps-legends.png", width = 80, height=180, units=c("mm"), 
       dpi = 600)
